...
(function() {
  ...
  try {
    ...
    compilers.styl = function(_path) {
      ...
      result = '';
      stylus(content).include(path.dirname(_path)).set('include css', true).set('line-numbers', compilers.DEBUG).set('firebug', (__indexOf.call(process.argv, '--firebug') >= 0)).render(function(err, css) {
        if (err) {
          throw err;
        }
        return result = css;
