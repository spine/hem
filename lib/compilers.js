...
(function() {
  var compileCoffeescript, compilers, cs, eco, err, fs, jade, path, stylus, utils,
  ...
  try {
    ...
    compilers.styl = function(_path) {
      ...
      result = '';
      stylus(content).include(path.dirname(_path)).set('include css', (__indexOf.call(process.argv, '--includeCss') >= 0)).set('compress', utils.COMPRESS).render(function(err, css) {
        if (err) {
          throw err;
        }
        return result = css;
