...
(function() {
  ...
  try {
    ...
    compilers.styl = function(path) {
      ...
      content = fs.readFileSync(path, 'utf8');
      result = '';
      stylus(content).include(dirname(path)).set('include css', true).set('line-numbers', (__indexOf.call(process.argv, '-d') >= 0) || (__indexOf.call(process.argv, '--debug') >= 0)).set('firebug', (__indexOf.call(process.argv, '--firebug') >= 0)).render(function(err, css) {
        if (err) {
          throw err;
        }
        return result = css;
