...
(function() {
  var compilers, cs, eco, fs, jade, path, stylus,
  ...
  try {
    ...
    compilers.styl = function(_path) {
      ...
      result = '';
      stylus(content).include(path.dirname(_path)).set('include css', (__indexOf.call(process.argv, '--includeCss') >= 0)).set('line-numbers', compilers.DEBUG).set('compress', !compilers.DEBUG).render(function(err, css) {
        if (err) {
          throw err;
        }
        return result = css;
