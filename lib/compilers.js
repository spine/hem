...
(function() {
  ...
  try {
    ...
    compilers.styl = function(path) {
      ...
      content = fs.readFileSync(path, 'utf8');
      result = '';
      stylus(content).include(dirname(path)).set('include css', (__indexOf.call(process.argv, '--includeCss') >= 0)).set('line-numbers', (__indexOf.call(process.argv, '-d') >= 0) || (__indexOf.call(process.argv, '--debug') >= 0)).set('compress', !(__indexOf.call(process.argv, '-d') >= 0) && !(__indexOf.call(process.argv, '--debug') >= 0)).render(function(err, css) {
        if (err) {
          throw err;
        }
        return result = css;
