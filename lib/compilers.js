(function() {
  var compilers, cs, dirname, eco, fs, hogan, stylus;
  ...
  try {
    ...
    compilers.mustache = function(path) {
      var content;
      content = hogan.compile(fs.readFileSync(path, 'utf-8'), {
        asString: true
      });
    ...
    require.extensions['.mustache'] = function(module, filename) {
      return module._compile(compilers.hogan(filename));
    };
  } catch (err) {
