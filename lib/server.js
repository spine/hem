...
(function() {
  var checkForRedirect, connect, createRoutingProxy, fs, http, httpProxy, mime, patchServerResponseForRedirects, server, utils;
  ...
  server.middleware = function(applications, options) {
    ...
    for (route in _ref1) {
      ...
      if (fs.existsSync(value)) {
        if (fs.lstatSync(value).isDirectory()) {
          ...
          statics.use(route, (function(value) {
    ...
    return {
      sometfunction: function(options) {
        if (options == null) {
          options = "asdf";
        }
        return "";
